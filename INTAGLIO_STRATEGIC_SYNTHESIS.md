...
> **Intaglio = the treasury control plane for crypto-native AI operators.**
...
| That company is Intaglio specifically | 22% | **13%** | Microsoft Agent 365 lands directly on "control plane" framing. A pure-play startup wins only with a niche wedge. |
...
2. **Regulation is a forcing function, not hype.** EU AI Act Article 12 (mandatory event logging across lifecycle) is in force. Articles 14/15/Annex III classify financial-decision agents as high-risk. MiCA's July 2026 deadline creates a compliance scramble for any EU company running agents with money. Intaglio can sell "one-button EU compliance" as a wedge feature.
...
## Top 3 risks that could kill Intaglio in 10 years + defenses
...
2. **Ramp extends agent policy to crypto via partnership (e.g., acquires Crossmint / Skyfire).**
   - *Likelihood: 40%.* Ramp has the cash ($500M recent raise) and the customer base. This is the most likely kill-shot. *Defense:* own the EU side hard — Ramp is US-centric and will struggle with MiCA / AI Act compliance narrative. Also, partnership-first, not competition-first: Intaglio could become the Ramp-for-crypto-agents OEM layer in 12-18 months.

3. **AP2 + MPP + x402 consolidate into a single standard with built-in policy primitives.**
   - *Likelihood: 30%.* Google AP2 already frames itself as "governance and trust" and has 60+ partners. If the protocol absorbs policy, Intaglio loses its reason to exist. *Defense:* build on top of AP2, don't fight it. Become the enterprise enforcement layer for AP2 policies — same relationship Okta has to OAuth. The protocol is a primitive; enterprise-grade implementation is still a SaaS.

## The specific wedge product feature set for the 60-day build

**Intaglio v0 — "Agent Treasury" (Solana-first, chain-agnostic architecture)**
...
## The 15 customer discovery calls — see `INTAGLIO_CUSTOMER_DISCOVERY_PLAN.md`

## Superteam pitch — see `INTAGLIO_SUPERTEAM_PITCH.md`

## 60-day build plan — see `INTAGLIO_60_DAY_BUILD_PLAN.md`

## Oracle insight — see `INTAGLIO_ORACLE_NOTE.md`
