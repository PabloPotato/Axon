...
//     and then the data. A reader who has never used Axon must understand.
...
import { hashPolicy } from "@axon/engine";
...
export async function renderAuditPDF(opts: RenderOptions): Promise<void> {
  const doc = new PDFDocument({
    ...
    margin: 56,
    info: {
      Title: `Axon Audit Report — ${opts.policy.id}`,
...
function renderCover(doc: PDFKit.PDFDocument, opts: RenderOptions): void {
  ...
  doc.fillColor(COLOR.muted).fontSize(10).text("AXON AUDIT REPORT v0.1", { align: "left" });
  doc.moveDown(0.4);
  rule(doc);
  ...
  doc
    ...
    .font("Helvetica")
    .text(
      "Hash-chained evidence of AI-agent actions under a published Axon Policy Language (APL) policy.",
      { align: "left" },
    );
  ...
  doc
    ...
    .font("Helvetica")
    .text(
      "This report was produced by axon-audit, an open-source (MIT) conformance implementation of the Axon Policy Language (APL, CC-BY-4.0). Any third party can independently re-verify the hash chain using the JSON export referenced in the Chain Integrity section.",
      { align: "left" },
    );
}
...
function renderIntegrity(doc: PDFKit.PDFDocument, opts: RenderOptions): void {
  ...
  para(
    doc,
    "1. Export the record stream via the axon-audit CLI as records.json.\n" +
      "2. Run axon-audit verify records.json — exit code 0 iff the chain is intact.\n" +
...
function renderFrameworkMapping(doc: PDFKit.PDFDocument): void {
  ...
  para(
    doc,
    "This section maps the fields of each Axon audit record to the specific articles of the three EU frameworks most relevant to stablecoin-paying AI agents in 2026: the AI Act, MiCA, and DORA. The mapping is informational; final sufficiency determination rests with the operator's legal counsel.",
  );
  ...
  for (const framework of ["EU AI Act", "MiCA", "DORA"] as const) {
    ...
    for (const req of requirementsByFramework(framework)) {
      ...
      doc
        .fillColor(COLOR.muted)
        .fontSize(10)
        .text(`Axon evidence: ${req.axon_evidence}`);
      doc
        .fillColor(COLOR.muted)
        .fontSize(9)
        .font("Courier")
...
function renderSignatureBlock(doc: PDFKit.PDFDocument, opts: RenderOptions): void {
  ...
  para(
    doc,
    "By signing below, the operator attests that the enclosed record stream was produced by a deployment of axon-engine operating under the policy whose hash appears on the cover page, that no records have been removed, and that the operator is authorised to certify this report on behalf of the organisation.",
  );
